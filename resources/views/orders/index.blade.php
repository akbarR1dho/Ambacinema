...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 border-b border-slate-200 pb-4 gap-4">
        ...
        <form action="{{ route('orders.index') }}" method="GET" class="w-full sm:w-auto" id="filterForm">
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <label for="filter-date" class="text-sm font-medium text-slate-500 whitespace-nowrap hidden sm:block">Time Range:</label>
                <select name="date_filter" id="filter-date" class="w-full sm:w-auto px-4 py-2 bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 shadow-sm" onchange="document.getElementById('filterForm').submit()">
                    <option value="all" {{ request('date_filter', 'today') == 'all' ? 'selected' : '' }}>All Time</option>
                    <option value="today" {{ request('date_filter', 'today') == 'today' ? 'selected' : '' }}>Today</option>
                    <option value="weekly" {{ request('date_filter', 'today') == 'weekly' ? 'selected' : '' }}>This Week</option>
                    <option value="monthly" {{ request('date_filter', 'today') == 'monthly' ? 'selected' : '' }}>This Month</option>
                    <option value="annual" {{ request('date_filter', 'today') == 'annual' ? 'selected' : '' }}>This Year</option>
                </select>
            </div>
        </form>
    </div>
    ...
    @if($orders->count() > 0)
        ...
        <div id="loading-indicator" class="hidden text-center py-6 mt-4">
            ...
            <p class="text-sm text-slate-500 mt-2">Loading more tickets...</p>
        </div>
    @else
